...
function BoutiquePage() {
  ...
  const displayedProducts = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (sortBy === 'recent-asc') return new Date(a.addedAt) - new Date(b.addedAt)
    ...
    <div className="min-h-screen bg-sable text-anthracite">
      ...
      <main className="px-4 pb-14 pt-8 md:px-[90px] md:pb-[80px] md:pt-[52px]">
        <section className="mx-auto max-w-[1260px]">
          <p className="font-sacramento text-[54px] leading-[0.9] text-moutarde">collections signatures</p>
          ...
          <p className="mt-3 max-w-[680px] font-rubik text-[16px] leading-[1.65] text-anthracite/78">
            Des accessoires <strong>faits main</strong>, conçus en petites séries, pour composer un vestiaire canin
            chic, confortable et durable.
          </p>

          <div className="mt-6 relative w-full md:max-w-[440px]">
            <Search size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-anthracite/50" />
            <input
              type="text"
              value={searchQuery}
              onChange={(event) => setSearchQuery(event.target.value)}
              placeholder="Rechercher un produit, une catégorie..."
              className="h-11 w-full rounded-full border border-anthracite/15 bg-white pl-10 pr-4 font-rubik text-[13px] text-anthracite outline-none placeholder:text-anthracite/45"
            />
          </div>

          <div className="mt-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="flex flex-wrap gap-2">
            {['Tout voir', 'Harnais', 'Snoods', 'Laisses', 'Colliers'].map((chip, index) => (
              <button
                key={chip}
            ...
            <div className="flex w-full items-center justify-between gap-3 md:w-auto md:justify-end">
              ...
              <div className="relative">
                <select
                  ...
                  className="h-11 rounded-full border border-anthracite/15 bg-white pl-4 pr-12 font-rubik text-[11px] uppercase tracking-[0.14em] text-anthracite outline-none appearance-none"
                >
                  <option value="none">Aucun tri</option>
                  <option value="recent-desc">Plus récent au plus ancien</option>
                  <option value="recent-asc">Plus ancien au plus récent</option>
                  <option value="price-asc">Moins cher au plus cher</option>
                  <option value="price-desc">Plus cher au moins cher</option>
                </select>
                <ChevronDown
                  size={14}
                  className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-anthracite/65"
                />
              </div>
            </div>
          </div>
        ...
        <section className="mx-auto mt-8 max-w-[1260px]">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4">
            {displayedProducts.map((product, index) => (
              <article key={`${product.name}-${index}`} className="rounded-[22px] border border-anthracite/[0.08] bg-white p-3">
                <div className="relative">
                  <img src={withBase(product.image)} alt={product.name} className="h-52 w-full rounded-[14px] object-cover" />
                  <span className="absolute left-2 top-2 rounded-full bg-sable/95 px-2 py-[5px] font-rubik text-[9px] uppercase tracking-[0.14em] text-[#728B75]">
                    {product.badge}
                  </span>
                </div>
                <p className="mt-4 font-rubik text-[10px] uppercase tracking-[0.17em] text-[#728B75]">{product.category}</p>
                <h2 className="min-h-[48px] font-cormorant text-[30px] leading-[1.02] text-anthracite">{product.name}</h2>
          ...
          {displayedProducts.length === 0 && (
            <p className="mt-6 text-center font-rubik text-[14px] text-anthracite/65">
        ...
        <section className="mx-auto mt-10 max-w-[1260px] rounded-[24px] border border-anthracite/[0.08] bg-vieuxrose px-6 py-6 md:px-8">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="font-sacramento text-[42px] leading-[0.9] text-moutarde">pièces d'atelier</p>
              <p className="font-rubik text-[15px] text-anthracite/76">
                Livraison suivie, retours 14 jours, et conseils taille pour chaque modèle.
              </p>
            </div>
            <a
              href={withBase('/product')}
